...
surfaces = parser.convertMech(inputFile=reactions_filepath,
                              ...
                              surfaceFile=None,
                              phaseName=None,
                              outName='~/Code/OOHAbstraction/comparisonTST/reference_files/master_heptane.cti',
                              permissive=True)
...
outdir = '~/Code/OOHAbstraction/comparisonTST/reference_files/cantera_sub_models/heptane/'
...
def get_ignition_delay(cantera_file_path, temperature , pressure, stoichiometry=1.0, isomer='N'):
    ...
    for the heptane isomer (n,s,t,i)
    """
    try:
    ...
    oxygen_mole = 1.0
    argon_mol = np.nan # we don't care about argon for this model
...
if not os.path.exists("~/Code/OOHAbstraction/comparisonTST/reference_files/ignition_delay_heptane.pkl"):
    for cantera_file in os.listdir("~/Code/OOHAbstraction/comparisonTST/reference_files/cantera_sub_models/heptane/"):
        identifier = cantera_file.split(".")[1]
        print
        print cantera_file
        delays = []
        for ignition_temp in ignition_temps:
            delay = get_ignition_delay(os.path.join("~/Code/OOHAbstraction/comparisonTST/reference_files/cantera_sub_models/heptane/", cantera_file),
    ...
    f=open("~/Code/OOHAbstraction/comparisonTST/reference_files/ignition_delay_heptane.pkl", "w")
    pickle.dump(dff, f)
else:
