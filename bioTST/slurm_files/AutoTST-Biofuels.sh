...
cd /gss_gpfs_scratch/harms.n/bioTST/ # Edit this as need be
# the "stdbuf -o0 -e0"  and the "-u" are to disable buffering,
# so that you see output from the script in the log files immediately.
stdbuf -o0 -e0 python -u ~/Code/OOHabstraction/bioTST/python_files/biofuelsTST.py > /gss_gpfs_scratch/harms.n/bioTST/biofuels_rerun_Nov2/AutoTST-biofuels.$SLURM_ARRAY_TASK_ID.combined.log 2>&1
